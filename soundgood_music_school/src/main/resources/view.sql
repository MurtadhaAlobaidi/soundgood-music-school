...
CREATE VIEW rented_instruments_info AS
    SELECT 
        ...
        ,rented_instruments.renting_status
        ,rent_date
        ,return_date
        ,instrument.type
        ,instrument.brand
...
--Task4_3)
CREATE VIEW terminated_task AS
    SELECT  
    rented_instruments.instrument_id
    ,student.student_id
    ,instrument.status
    ,rented_instruments.renting_status
    ,student.total_rented_instruments_currently
FROM student 
INNER JOIN rented_instruments ON rented_instruments.student_id=student.student_id
INNER JOIN instrument ON instrument.instrument_id=rented_instruments.instrument_id; 
